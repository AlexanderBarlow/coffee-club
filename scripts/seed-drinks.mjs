...
// Helper: randomly select up to maxItems from an array
function getRandomItems(array, maxItems) {
  const count = randomInt(0, maxItems + 1); // can be 0, 1, 2...
...
async function main() {
  ...
  for (const drink of drinks) {
    const randomSyrups = getRandomItems(syrupRecords, 2); // Up to 2 syrups
    const randomSauces = getRandomItems(sauceRecords, 2); // Up to 2 sauces
