...
// Size abbreviation to full word map
const SIZE_MAP = {
  S: "Small",
  M: "Medium",
  L: "Large",
...
export default function TicketingPage() {
  ...
  const handleClaim = async (orderId) => {
    ...
    try {
      const res = await fetch(`/api/admin/orders/${orderId}/claim`, {
        method: "PATCH",
      });
      ...
      if (res.ok) {
        ...
        console.error("Failed to claim order");
      }
    } catch (err) {
                      ...
                      {order.items.map((item, index) => (
                        <Box key={index} sx={{ mb: 1 }}>
                          <Typography variant="body1" fontWeight={600}>
                            ...
                            {SIZE_MAP[item.customization?.size] ||
                          ...
                          <Typography variant="body2" color="text.secondary">
                            Milk: {item.customization?.milk || "Default"}
                          </Typography>

                          {item.customization?.syrups && (
                            <Typography variant="body2" color="text.secondary">
                              Syrups: {item.customization.syrups.join(", ")}
                            </Typography>
                          )}
